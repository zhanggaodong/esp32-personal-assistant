...
extern const uint8_t font_custom_20_4_bin_start[] asm("_binary_font_custom_20_4_bin_start");
...
LcdDisplay::LcdDisplay(esp_lcd_panel_io_handle_t panel_io, esp_lcd_panel_handle_t panel, int width, int height)
    : panel_io_(panel_io), panel_(panel),
      sleep_container_(nullptr), sleep_time_label_(nullptr),
      sleep_date_label_(nullptr), sleep_week_label_(nullptr),
      sleep_battery_label_(nullptr), sleep_timer_(nullptr) {
    width_ = width;
    height_ = height;
    ...
    if (current_theme_ == nullptr) {
        ESP_LOGW(TAG, "Theme '%s' not found, using 'light' theme", theme_name.c_str());
...
LcdDisplay::~LcdDisplay() {
    ...
    if (sleep_time_font_ != nullptr) {
        cbin_font_delete(sleep_time_font_);
    }
    if (sleep_date_font_ != nullptr) {
        cbin_font_delete(sleep_date_font_);
    }

    if (sleep_container_ != nullptr) {
        lv_obj_del(sleep_container_);
    }
...
void LcdDisplay::SetupUI() {
    ...
    // Modified: Increased padding to move icons inward
    lv_obj_set_style_pad_left(top_bar_, lvgl_theme->spacing(10), 0);
    lv_obj_set_style_pad_right(top_bar_, lvgl_theme->spacing(10), 0);
...
void LcdDisplay::SetupUI() {
    ...
    // Modified: Increased padding to move icons inward
    lv_obj_set_style_pad_left(top_bar_, lvgl_theme->spacing(20), 0);
    lv_obj_set_style_pad_right(top_bar_, lvgl_theme->spacing(20), 0);
    ...
    // Modified: Increased bottom padding to lift the text up from the edge
    lv_obj_set_style_pad_bottom(bottom_bar_, lvgl_theme->spacing(20), 0);
    ...
    // Modified: Changed from WRAP to SCROLL_CIRCULAR for marquee effect on one line
...
void LcdDisplay::SetTheme(Theme* theme) {
    ...
    // Update sleep screen if it exists
    if (sleep_container_) {
        lv_obj_set_style_bg_color(sleep_container_, lvgl_theme->background_color(), 0);
        // lv_obj_set_style_pad_row(sleep_container_, lvgl_theme->spacing(2), 0);

        if(sleep_time_label_) {
           lv_obj_set_style_text_color(sleep_time_label_, lvgl_theme->text_color(), 0);
        }
        if(sleep_date_label_) {
           lv_obj_set_style_text_color(sleep_date_label_, lvgl_theme->text_color(), 0);
        }
        if(sleep_week_label_) {
           lv_obj_set_style_text_color(sleep_week_label_, lvgl_theme->text_color(), 0);
        }
        if(sleep_battery_label_) {
           lv_obj_set_style_text_color(sleep_battery_label_, lvgl_theme->text_color(), 0);
        }
    }
...
void LcdDisplay::SetPowerSaveMode(bool on) {
    ...
    if (on) {
        // Check if theme is valid before proceeding
        if (current_theme_ == nullptr) {
            ESP_LOGW(TAG, "Cannot enter power save mode: current_theme_ is null");
            return;
        }
        ...
        // Create sleep container if needed
        if (sleep_container_ == nullptr) {
            auto lvgl_theme = static_cast<LvglTheme*>(current_theme_);
            
            // Lazy load fonts
            if (sleep_time_font_ == nullptr) {
                sleep_time_font_ = cbin_font_create((uint8_t*)font_custom_80_4_bin_start);
            }
            if (sleep_date_font_ == nullptr) {
                sleep_date_font_ = cbin_font_create((uint8_t*)font_custom_20_4_bin_start);
            }

            sleep_container_ = lv_obj_create(lv_screen_active());
            lv_obj_set_size(sleep_container_, LV_HOR_RES, LV_VER_RES);
            lv_obj_set_style_bg_color(sleep_container_, lvgl_theme->background_color(), 0);
            lv_obj_set_style_border_width(sleep_container_, 0, 0);
            lv_obj_set_style_radius(sleep_container_, 0, 0);
            lv_obj_set_flex_flow(sleep_container_, LV_FLEX_FLOW_COLUMN);
            lv_obj_set_flex_align(sleep_container_, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER, LV_FLEX_ALIGN_CENTER);
            lv_obj_set_style_pad_all(sleep_container_, 0, 0);
            lv_obj_set_style_pad_row(sleep_container_, 0, 0);

            // Time: Custom 60px Font
            sleep_time_label_ = lv_label_create(sleep_container_);
            lv_obj_set_style_text_font(sleep_time_label_, sleep_time_font_, 0); 
            lv_obj_set_style_text_color(sleep_time_label_, lvgl_theme->text_color(), 0);

            // Date: Custom 20px Chinese Font
            sleep_date_label_ = lv_label_create(sleep_container_);
            lv_obj_set_style_text_font(sleep_date_label_, sleep_date_font_, 0);
            lv_obj_set_style_text_color(sleep_date_label_, lvgl_theme->text_color(), 0);
            lv_obj_set_style_margin_top(sleep_date_label_, lvgl_theme->spacing(6), 0);
            
            // Week: Custom 20px Chinese Font
            sleep_week_label_ = lv_label_create(sleep_container_);
            lv_obj_set_style_text_font(sleep_week_label_, sleep_date_font_, 0);
            lv_obj_set_style_text_color(sleep_week_label_, lvgl_theme->text_color(), 0);
            lv_obj_set_style_margin_top(sleep_week_label_, lvgl_theme->spacing(2), 0);
            
            // Battery Icon
            sleep_battery_label_ = lv_label_create(sleep_container_);
            auto icon_font = lvgl_theme->icon_font();
            lv_obj_set_style_text_font(sleep_battery_label_, icon_font ? icon_font->font() : &BUILTIN_ICON_FONT, 0);
            lv_obj_set_style_text_color(sleep_battery_label_, lvgl_theme->text_color(), 0);
            lv_obj_set_style_margin_top(sleep_battery_label_, lvgl_theme->spacing(2), 0);
        } else {
             lv_obj_remove_flag(sleep_container_, LV_OBJ_FLAG_HIDDEN);
        }

        UpdateSleepScreenLocked();

        // Create timer to update at the start of each minute
        if (sleep_timer_ == nullptr) {
            esp_timer_create_args_t timer_args = {
                .callback = [](void* arg) {
                    LcdDisplay* display = static_cast<LcdDisplay*>(arg);
                    display->UpdateSleepScreen();
                    
                    // After first one-shot trigger, restart as periodic timer aligned to minutes
                    if (!esp_timer_is_active(display->sleep_timer_)) {
                        // This was a one-shot, now start periodic
                        esp_timer_start_periodic(display->sleep_timer_, 60 * 1000 * 1000); // 1 minute
                    }
                },
                .arg = this,
                .dispatch_method = ESP_TIMER_TASK,
            ...
            esp_timer_create(&timer_args, &sleep_timer_);
            
            // Calculate time until next whole minute to align updates
            time_t now = time(NULL);
            struct tm tm_buf;
            struct tm* tm = localtime_r(&now, &tm_buf);
            int seconds_to_next_minute = 60 - (tm ? tm->tm_sec : 0);
            if (seconds_to_next_minute <= 0) seconds_to_next_minute = 60;
            
            // Start with one-shot to align to the next minute, then periodic callback will restart as periodic
            esp_timer_start_once(sleep_timer_, seconds_to_next_minute * 1000 * 1000);
        }
        ...
        if (emoji_image_ && !lv_obj_has_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN)) {
            // If label was hidden (meaning image was shown), show image
            // Actually this logic is tricky because we don't know previous state perfectly.
            // But usually we just unhide everything and let SetEmotion sort it out later if needed.
            // Or just unhide image if label is hidden.
            bool image_was_active = lv_obj_has_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
            if (image_was_active) {
                lv_obj_remove_flag(emoji_image_, LV_OBJ_FLAG_HIDDEN);
            } else {
                lv_obj_remove_flag(emoji_label_, LV_OBJ_FLAG_HIDDEN);
        ...
        // Clean up sleep container completely to avoid issues on restart
        if (sleep_container_) {
            lv_obj_del(sleep_container_);
            sleep_container_ = nullptr;
            sleep_time_label_ = nullptr;
            sleep_date_label_ = nullptr;
            sleep_week_label_ = nullptr;
            sleep_battery_label_ = nullptr;
        }
...
// This method is called by the timer (unlocked context), so it needs to take the lock
void LcdDisplay::UpdateSleepScreen() {
    DisplayLockGuard lock(this);
    UpdateSleepScreenLocked();
}

// This helper performs the actual update, assuming the lock is already held
void LcdDisplay::UpdateSleepScreenLocked() {
    if (!sleep_container_ || lv_obj_has_flag(sleep_container_, LV_OBJ_FLAG_HIDDEN)) return;
    ...
    if (sleep_time_label_) {
        ...
        lv_label_set_text(sleep_time_label_, time_str);
    }

    // Date: 2025年12月31日
    if (sleep_date_label_) {
        char date_str[64];
        snprintf(date_str, sizeof(date_str), "%d年%02d月%02d日", tm->tm_year + 1900, tm->tm_mon + 1, tm->tm_mday);
        lv_label_set_text(sleep_date_label_, date_str);
    }

    // Week: 星期三
    if (sleep_week_label_) {
        const char* week_days[] = {"星期日", "星期一", "星期二", "星期三", "星期四", "星期五", "星期六"};
        lv_label_set_text(sleep_week_label_, week_days[tm->tm_wday]);
    }
    ...
    if (sleep_battery_label_) {
        ...
        lv_label_set_text(sleep_battery_label_, icon);
    }
}
