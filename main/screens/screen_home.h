...
private:
    ...
    // LVGL 对象
    void* wallpaper_img_ = nullptr;   // lv_obj_t*（壁纸图层）
    void* title_label_ = nullptr;     // lv_obj_t*（主标题）
    void* subtitle_label_ = nullptr;  // lv_obj_t*（副标题）

    // 壁纸解码缓冲（RGB565，需 heap_caps_free）
    uint8_t* wallpaper_buf_ = nullptr;
    int wallpaper_w_ = 0;
    int wallpaper_h_ = 0;
    void* wallpaper_dsc_ = nullptr;  // lv_img_dsc_t*（指向 wallpaper_buf_, 供 lv_image 引用）
};