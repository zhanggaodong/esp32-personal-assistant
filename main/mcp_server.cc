...
void McpServer::AddCommonTools() {
    ...
    if (music) {
        AddTool("self.music.search",
            "Search and play music by song name and artist name.",
            PropertyList({
                Property("song", kPropertyTypeString),
                Property("artist", kPropertyTypeString, "")
            }),
            [music](const PropertyList& properties) -> ReturnValue {
                std::string song = properties["song"].value<std::string>();
                std::string artist = properties["artist"].value<std::string>();
                return music->Download(song, artist);
            });
    }

    AddTool("self.timer.start",
        "Start a countdown timer for a specified number of seconds.",
        PropertyList({
        ...
        [](const PropertyList& properties) -> ReturnValue {
            ...
            app.Schedule([seconds, &app]() {
                app.StartCountdown(seconds);
            });
            return true;
        });

    AddTool("self.timer.stop",
